 ...
 */
 ...
 {
  if ((!$this->upstream) || (method_exists($this->upstream,'gracefulShutdown')?$this->upstream->gracefulShutdown():true))
  {
   $this->finish();
   return TRUE;
  }
 ...
 */
 public function computeKey($key)
 {
  ...
  if ($spaces > 0) {$result = (int) floor($digits / $spaces);}
  else {$result = (float) $digits;}
